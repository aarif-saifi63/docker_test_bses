...
# CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=True)  # Enables CORS for all routes
CORS(app, supports_credentials=True, resources={r"/*": {"origins": "*"}})  # Enables CORS for all routes
...
def add_security_headers(response):
    ...
    # SECURITY: Removed 'unsafe-inline' and 'unsafe-eval' to prevent SVG XSS attacks
    csp_policy = (
        "default-src 'self'; "
        "script-src 'self'; "
...
@app.route("/mappings/<int:mapping_id>", methods=["PUT"])
# @token_required
...
@app.route("/language/<int:language_id>", methods=["PUT"])
@token_required
...
@app.route("/language/<int:language_id>", methods=["DELETE"])
@token_required
...
@app.route("/utter_messages/<uuid:uid>", methods=["DELETE"])
@token_required
