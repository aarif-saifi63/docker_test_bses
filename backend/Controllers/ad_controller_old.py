...
def secure_upload(file, category):
    """
    SECURITY: Enhanced secure upload function with comprehensive validations

    Uses FileSecurityHelper for robust file validation including:
    - File size validation
    - Magic number (file signature) validation
    - MIME type validation
    - Double extension detection
    - Malicious content scanning
    - PDF script/JavaScript detection (VAPT requirement)
    ...
    # SECURITY: Use FileSecurityHelper for comprehensive validation
    is_valid, error_message = FileSecurityHelper.is_valid_file(file, content_type, max_size)
    ...
    # SECURITY: Verify MIME type after saving (defense in depth)
    if not validate_mime(path, category):
        try:
            os.remove(path)
        except Exception:
            pass
        return False, None, "MIME type verification failed"
