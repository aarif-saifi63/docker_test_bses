...
# SECURITY: Restricted file types for icons - SVG allowed with strict validation
ALLOWED_EXTENSIONS = {"png", "jpg", "jpeg", "svg"}

# File size limit for icons - SECURITY: Prevent DoS via large files
MAX_FILE_SIZE = 5 * 1024 * 1024  # 5 MB

# MIME type validation for icons - SECURITY: Validate actual file content
ALLOWED_MIME_TYPES = {
    "image/png",
    "image/jpeg",
    "image/svg+xml"
...
# File signatures for icons - SECURITY: Validate magic bytes
MAGIC_SIGNATURES = {
    "png": [b"\x89PNG\r\n\x1a\n"],
    "jpg": [b"\xff\xd8\xff"],
    "jpeg": [b"\xff\xd8\xff"],
...
#         # --- create folder ---
#         menu_folder_path = os.path.join(ICON_BASE_PATH)
...
#                 if menu_icon_file:
#                     # SECURITY: Validate and save icon with proper error handling
#                     success, icon_path, error = _save_icon_file(menu_folder_path, menu_icon_file)
#                     if not success:
#                         return jsonify({
#                             "status": False,
#                             "message": "Menu icon upload failed",
#                             "error": error,
#                             "file": menu_icon_file.filename
#                         }), 400
...
#                     lang=lang,
#                     is_visible=is_visible,
#                     menu_sequence=db.query(MenuOptionV).filter(MenuOptionV.user_id == user_id).count() + 1,
...
#             #     submenu_icon_path = _save_icon_file(menu_folder_path, submenu_icon_file) if submenu_icon_file else None

#             #     if submenu_id:
#             #         submenu = db.query(SubMenuOptionV).filter_by(id=submenu_id, user_id=user_id).first()
#             #         if submenu:
...
#             # Get current count for assigning new sequence numbers
...
#                 # SECURITY: Validate and save submenu icon with proper error handling
#                 submenu_icon_path = None
#                 if submenu_icon_file:
#                     success, icon_path, error = _save_icon_file(menu_folder_path, submenu_icon_file)
#                     if not success:
#                         return jsonify({
#                             "status": False,
#                             "message": f"Submenu icon upload failed for '{submenu_name}'",
#                             "error": error,
#                             "file": submenu_icon_file.filename,
#                             "submenu_index": idx
#                         }), 400
#                     submenu_icon_path = icon_path
...
#                 else:
#                     # Create new submenu with next available sequence
#                     submenu = SubMenuOptionV(
#                         menu_id=menu.id,
#                         user_id=user_id,
#                         name=submenu_name,
...
#                 # No need to flush after each submenu, just once after the loop
#                 db.flush()
...
#                     # Delete steps that are no longer in incoming
                ...
                if menu_id:
                    ...
                    if menu_icon_file:
                        ...
                        if not success:
                            return jsonify({
                                "status": False,
                                "message": f"Menu icon upload failed for {error}",
                                "error": "Menu icon upload failed",
                                "file": menu_icon_file.filename
                            }), 400
                        menu.icon_path = icon_path
                else:
                ...
                for idx, submenu_data in enumerate(submenus_payload):
                    ...
                    if submenu_icon_file:
                        ...
                        if not success:
                            return jsonify({
                                "status": False,
                                "message": f"Submenu icon upload failed for {error}",
