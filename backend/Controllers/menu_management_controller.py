...
def create_menu_with_submenu():
    ...
    try:
        ...
        # ============================================================
        # CRITICAL FIX: Validate ALL files BEFORE starting transaction
        # This ensures atomicity - either all files are valid or none are uploaded
        # ============================================================

        menu_icon_file = request.files.get("menu_icon")
        validated_files = {}  # Store validated file data for later upload

        # Validate menu icon if provided
        if menu_icon_file and menu_icon_file.filename:
            is_valid, error = FileSecurityHelper.is_valid_file(
                menu_icon_file,
                menu_icon_file.content_type,
                max_size=5 * 1024 * 1024  # 5MB for images
            )
            if not is_valid:
                return jsonify({
                    "status": False,
                    "message": f"Menu icon validation failed: {error}"
                }), 400
            validated_files['menu_icon'] = menu_icon_file

        # Validate all submenu icons if provided
        for idx, submenu_data in enumerate(submenus_payload):
            submenu_name = submenu_data.get("name", f"submenu_{idx}")
            submenu_icon_file = request.files.get(f"submenu_icon_{idx}")

            if submenu_icon_file and submenu_icon_file.filename:
                is_valid, error = FileSecurityHelper.is_valid_file(
                    submenu_icon_file,
                    submenu_icon_file.content_type,
                    max_size=5 * 1024 * 1024  # 5MB for images
                )
                if not is_valid:
                    return jsonify({
                        "status": False,
                        "message": f"Submenu icon upload failed for '{submenu_name}': {error}",
                        "error": f"Submenu icon upload failed for '{submenu_name}'",
                        "file": submenu_icon_file.filename,
                        "submenu_index": idx
                    }), 400
                validated_files[f'submenu_icon_{idx}'] = submenu_icon_file

        # All files validated successfully - now we can proceed with transaction
        ...
        try:
            with db.begin():
                ...
                if menu_id:
                    ...
                    if 'menu_icon' in validated_files:
                        # File already validated - just save it
                        success, icon_path, error = FileSecurityHelper.validate_and_save_file(
                            validated_files['menu_icon'],
                            upload_dir=menu_folder_path,
                            allowed_categories=["image"]
                        )
                        if not success:
                            # This should never happen as we pre-validated, but handle it anyway
                            raise Exception(f"Menu icon save failed: {error}")

                        # Track for cleanup on failure
                        full_path = os.path.join(menu_folder_path, os.path.basename(icon_path))
                ...
                for idx, submenu_data in enumerate(submenus_payload):
                    ...
                    if f'submenu_icon_{idx}' in validated_files:
                        # File already validated - just save it
                        success, icon_path, error = FileSecurityHelper.validate_and_save_file(
                            validated_files[f'submenu_icon_{idx}'],
                            upload_dir=menu_folder_path,
                            allowed_categories=["image"]
                        )
                        if not success:
                            # This should never happen as we pre-validated, but handle it anyway
                            raise Exception(f"Submenu icon save failed for '{submenu_name}': {error}")
            ...
            db.rollback()
            # CRITICAL: Cleanup uploaded files on transaction failure
        ...
        db.rollback()
        # CRITICAL: Cleanup uploaded files on any failure
        for file_path in uploaded_files:
            try:
                if os.path.exists(file_path):
                    os.remove(file_path)
