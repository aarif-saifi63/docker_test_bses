...
export const AuthProvider = ({ children }) => {
  ...
  const logout = async () => {
    try {
      // Use bare axios to bypass apiClientInstance interceptors
      // avoids deadlock when logout is triggered from 401 interceptor
  ...
  useEffect(() => {
    ...
    // Cross-tab logout sync — fires in other tabs when localStorage changes
    const handleStorageChange = (e) => {
      if (e.key === "user_details" && e.newValue === null) {
        setUser(null);
        window.location.href = "/login";
      }
    };
