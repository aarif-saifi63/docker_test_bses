...
export default function MenuModal({
  ...
  // 🧠 Whenever user changes, set language from that Rajdhani user's record
  useEffect(() => {
    if (localUserId) {
      const selectedUser = rajdhaniUsers.find((u) => u.id === Number(localUserId));
      if (selectedUser && selectedUser.lang) {
        onInputChange("lang", selectedUser.lang); // ✅ auto-update menu language
      }
    }
  }, [localUserId, rajdhaniUsers, onInputChange]);
  ...
  const handleNext = () => {
    if (!formData.name.trim()) return alert("Menu title is required!");
    if (!localUserId) return alert("Please select a Rajdhani user!");
    setStep(2);
  };
            ...
            <div className="space-y-5 animate-fadeIn">
              ...
              {/* <div>
                ...
                <input
                  type="text"
                  value={
                    rajdhaniUsers.find((ele) => ele.id === localUserId)?.lang === "hi"
                      ? "Hindi"
                      : "English"
                  }
                  disabled
