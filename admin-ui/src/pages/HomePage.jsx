...
import chatMascotsIcon from "../assets/mascots/bses-100.png";
...
export default function HomePage() {
    ...
    try {
      ...
      if (data?.response && data.response.heading?.length > 0) {
        ...
        if (data.response.buttons) {
          ...
          // Disable input if language selection buttons are shown
          if (data.response.buttons.some(btn => btn === "English" || btn === "हिंदी" || btn.toLowerCase() === "english")) {
            setIsInputDisabled(true);
          }
        } else {
          setButtons([]);
  ...
  const handleRegisterMenuRunFlow = async (userTypeParam = selectedUserType) => {
    ...
    try {
      ...
      // Store type_of_user if it exists in the response
      if (data?.type_of_user) {
  ...
  const sendMessage = async (msg, isUser = true, langOverride = null, directOption = null, isSilent = false, senderIdOverride = null) => {
    ...
    try {
      ...
      // Reset flag when OTP is validated (using text-based check)
      if (
      ...
      if (data && Array.isArray(data.buttons) && Array.isArray(data.heading)) {
        ...
        if (isFaqStyle) {
          ...
          // Check if this is a special menu response
            ...
            // Disable input if language selection buttons are shown
            if (updatedButtons.some(btn => btn === "English" || btn === "हिंदी" || btn.toLowerCase() === "english")) {
              setIsInputDisabled(true);
            }
            setGroupedButtons(null);
            setFaqGroups([]);
